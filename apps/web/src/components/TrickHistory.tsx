  ...
  return (
    <div
      className={cn(
        ...
        isWinner
          ? 'border-amber-400 ring-1 ring-amber-400/40 scale-110 z-10'
          : 'border-gray-300/40',
      )}
    >
      <span className={cn(
...
export function TrickHistory({ gameState, aiSeats }: TrickHistoryProps) {
  ...
  return (
    <div className={cn(
      'fixed top-32 right-2 z-20 transition-all duration-300',
      expanded ? 'w-[calc(100vw-1rem)] sm:w-[380px]' : 'w-auto',
    )}>
      {!expanded && (
        <button
          onClick={() => setExpanded(true)}
          className="glass-strong rounded-xl px-4 py-2.5 border border-white/8 hover:border-emerald-500/20 transition-all flex items-center gap-2 shadow-lg"
        >
      ...
      {expanded && (
        <div className="glass-strong rounded-2xl border border-white/8 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/8">
            <span className="text-sm font-bold text-emerald-300">📜 היסטוריה</span>
            <button
